...
import { Lock, CheckCircle2, XCircle, Clock, Zap, Loader2 } from 'lucide-react';
        ...
        <div className="mt-4 flex items-center justify-between">
           ...
           {canView && user?.subscription === SubscriptionTier.PREMIUM && (
             <button 
                ...
                className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-naija-green to-emerald-600 text-white text-xs font-bold rounded-lg shadow-sm hover:shadow-md hover:from-green-700 hover:to-emerald-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed transform active:scale-95"
             >
               {loadingAnalysis ? (
               ...
               {loadingAnalysis ? 'Analyzing...' : 'AI Insight'}
             </button>
           )}
        </div>

        {analysis && canView && (
            <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-100 text-sm text-gray-700 leading-relaxed animate-fade-in relative">
                <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-naija-green to-emerald-600 rounded-l-lg"></div>
                <h4 className="font-semibold text-gray-900 mb-1 flex items-center gap-1.5 text-xs uppercase tracking-wider">
                    <Zap size={12} className="text-naija-green fill-current"/> Gemini AI Analysis
                </h4>
                {analysis}
            </div>
        )}
